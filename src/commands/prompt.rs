...
pub async fn run(cmd: PromptCmd, json: bool, verbose: bool, override_cmd: Option<&str>) -> Result<()> {
    match cmd {
        PromptCmd::List { server, timeout } => {
            let session = open(&server, Duration::from_secs(timeout), verbose, override_cmd).await?;
            let prompts = session::list_prompts(&session).await?;
            session::close(session).await;
            print_prompt_list(&server, &prompts, json);
        }
        PromptCmd::Get { server, prompt, args, timeout } => {
            let session = open(&server, Duration::from_secs(timeout), verbose, override_cmd).await?;
...
async fn open(server: &str, timeout: Duration, verbose: bool, override_cmd: Option<&str>) -> Result<Session> {
